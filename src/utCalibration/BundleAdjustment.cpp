...
#include <utCalibration/2D3DPoseEstimation.h> //compute Pose
...
// {
	...
	// checkConsistency2 ( points3d, points2d, points2dWeights, camPoses, camMatrices );
	...
	// for (int localBundleIndex = 0; localBundleIndex < localBundleSizes.size(); ++localBundleIndex) {
		...
		// std::pair < Math::ErrorPose , double > estimate = 
			// multipleCameraBundleAdjustment (points3d, points2d, points2dWeights, camPoses, camMatrices, minCorrespondences, false,
	...
	)
{
	...
	std::pair < Math::ErrorPose , double > estimate = 
		multipleCameraBundleAdjustment (points3d, points2d, points2dWeights, camPoses, camMatrices, minCorrespondences, hasInitialPoseProvided,
			initialPose);
	pose = estimate.first;
